...
class GCN(nn.Module):
    ...
    def forward(self, regional_means, adj):
        adj_means = regional_means.repeat(self.block_num, 1, 1, 1).permute(
            1, 0, 2, 3) * adj.unsqueeze(3)
        adj_means = (1 - torch.eye(self.block_num).reshape(
            1, self.block_num, self.block_num, 1).to(self.device)) * adj_means
...
class GCNII(nn.Module):
    ...
    def forward(self, regional_means, adj, layer=1):
        """
        Args:
        ...
        """
        beta = math.log(self.lamda / layer + 1)
...
class GIN(nn.Module):
    ...
    def forward(self, regional_means, adj):
        ...
        aggregated_features = torch.bmm(
            adj, regional_means) + (1 + self.eps) * regional_means
...
class FAGCN(nn.Module):
    ...
    def __init__(self,
        ...
        self.beta = nn.Parameter(torch.tensor(alpha))
    ...
    def forward(self, regional_means, adj):
        ...
        # 计算单位矩阵 I
        Id = torch.eye(self.block_num).to(self.device)

        # 计算低通和高通滤波器矩阵
        adjL = (self.sigma + 1) * Id - L
        adjH = (self.sigma - 1) * Id + L
        ...
        # 加权融合
        output = self.alpha * xL + self.beta * xH
        return output
...
class GAT(nn.Module):
    ...
    def forward(self, h: torch.Tensor, adj_mat: torch.Tensor):
        ...
        adj_mat = adj_mat.unsqueeze(2).repeat(1, 1, self.n_heads,
                                              1)  # 复制到每个注意力头
        ...
        if self.is_concat:
            return attn_res.reshape(batch_size, n_nodes,
                                    self.n_heads * self.n_hidden)
        else:
