...
import math
import numpy as np
from .gcn_functions import GCN, GCNII, GAT, GIN, FAGCN, SGC
...
class RegionalMeans(nn.Module):
    ...
    def forward(self, x: torch.Tensor, index_oh: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            index_oh: one-hot 编码的超像素标签，
                形状为 [batch_size, block_num, height, width]。
        ...
        regional_means = torch.sum(index_oh * input_r, dim=(3, 4)) / (
...
class Adj(nn.Module):
    ...
    def forward(self, regional_means: torch.Tensor, c: int):
        regional_means_ = regional_means.repeat(self.block_num, 1, 1,
                                                1).permute(1, 2, 0, 3)
        regional_means_ = (regional_means_ -
                           regional_means.unsqueeze(1)).permute(0, 2, 1, 3)
        M = torch.mm(self.W, self.W.T)
        adj = torch.matmul(regional_means_.reshape(self.batch_size, -1, c), M)
        adj = torch.sum(adj * regional_means_.reshape(self.batch_size, -1, c),
                        dim=2).view(self.batch_size, self.block_num,
                                    self.block_num)
...
class GNNlayer(nn.Module):
    ...
    def _one_hot(self, index: torch.Tensor):
        index_oh = torch.zeros(self.batch_size, self.block_num, self.h,
    ...
    def _gnn_function(self):
        ...
        if self.gnn_name == "gin":
            return GIN(self.block_num, self.in_channels, self.out_channels,
                       self.device)
