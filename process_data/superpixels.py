...
# felzenszwalb
def felzenszwalbData(data: np.ndarray,
                     scale: float = 120,
                     sigma: float = 0.5,
                     min_size: int = 300):
    """
...
def lscData(data: np.ndarray, region_size: int = 30, ratio: float = 0.075):
    ...
    _ = cv2.bitwise_and(data, data, mask=lsc_contours)
    block_num = np.max(lsc_labels)
    return lsc_labels, block_num + 1


def slicsData(data: np.ndarray,
              algorithm_name: str,
              region_size: int = 30,
              ruler: float = 10.0):
    algorithms = {
    ...
    _ = cv2.bitwise_and(data, data, mask=slic_contours)
...
def showSuperpixels(data: np.ndarray, segments: np.ndarray,
                    save_png_path: str):
