...
class show_data:
    '''
        ...
        方法：
            ...
            显示数据集的 t-SNE
    '''
    def __init__(self, data:np.ndarray, label:np.ndarray , data_name:str, save_png_path:str, if_pca:bool=True, if_tsne:bool=True) -> None:
        h, w, c = data.shape
        self.h = h
        self.w = w
    ...
    def show_gt(self):
        ...
        plt.savefig(self.save_png_path + '/' + self.data_name + ' ground_truth.png')
        plt.close()
    ...
    def plot_pca(self):
        ...
        # data_pca 的值缩放到 0-1
        data_pca = (data_pca - data_pca.min()) / (data_pca.max() - data_pca.min()) # * 255
        plt.figure()
        plt.imshow(data_pca)
        plt.title('PCA of {}'.format(self.data_name))
        plt.savefig(self.save_png_path + '/'+ self.data_name + ' pca_data.png')
        plt.close()
    ...
    def plot_tsne(self):
        ...
        plt.savefig(self.save_png_path + '/' + self.data_name + ' tsne_data.png')
        plt.close()

def show_mask(mask:np.ndarray, label:np.ndarray, data_name:str, plot_name:str, png_path:str):
    plt.figure()
